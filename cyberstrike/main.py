...
def draw_glitch(img, intensity=0.05, alpha=255):
    if not img: return
    jx = int(max(1, 40 * intensity))
    temp_surf = img.copy()
    temp_surf.set_alpha(alpha)
    screen.blit(temp_surf, (random.randint(-jx, jx), random.randint(-4, 4)))
    chance = min(0.08, intensity * 0.4)
    if random.random() < chance:
        for _ in range(1):  # cap burst size
...
def reset_game():
    ...
    global last_spawn_times, game_state, space_enabled
    ...
    # Clear the lists so old enemies/bullets disappear
    bullets = []
    enemies = []
    
    # Reset spawn timers to the current time
    for key in last_spawn_times:
        last_spawn_times[key] = start_time
...
def main():
    global running, WIDTH, HEIGHT, FPS, CYAN, YELLOW, MAGENTA, WHITE, RED, BLACK, DARK_BLUE
    ...
    while running:
        ...
        if game_state == "START_MENU":
            ...
            # 0 → 1 smooth difficulty curve
            progress = min(1.0, elapsed / 40.0)  # full scaling over 2 minutes

            # smooth easing instead of linear ramp
            difficulty = progress * progress
            speed_mod = 1.0 + difficulty * 1.8
            ...
            for tier, ms in spawn_rates.items():
                ...
                spawn_progress = min(1.0, elapsed / 90.0)
                spawn_intensity = 1.0 + spawn_progress * 1.2
                if stability <= 0 and rig_factor > 0: spawn_intensity *= 2.0
                
                if current_ticks - last_spawn_times[tier] > (ms / spawn_intensity):
